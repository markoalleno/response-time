...
struct WeeklyDigestView: View {
    ...
    private func exportWeeklyMarkdown() -> String {
        ...
        var md: [String] = []
        md.append("# Weekly Response Time Report")
        md.append("**\(weekRangeLabel)**")
        md.append("")
        md.append("## Summary")
        md.append("- **Responses:** \(valid.count)")
        md.append("- **Median:** \(formatDuration(median))")
        if let fastest = latencies.first {
            md.append("- **Fastest:** \(formatDuration(fastest))")
        }
        if let slowest = latencies.last {
            md.append("- **Slowest:** \(formatDuration(slowest))")
        }
        
        let under30 = latencies.filter { $0 < 1800 }.count
        let pct = latencies.isEmpty ? 0 : Int(Double(under30) / Double(latencies.count) * 100)
        md.append("- **Under 30 min:** \(pct)%")
        
        md.append("")
        md.append("## Day by Day")
        let dailyData = computeDailyData()
        md.append("| Day | Median | Count |")
        md.append("|-----|--------|-------|")
        for day in dailyData where day.count > 0 {
            md.append("| \(day.day) | \(formatDuration(day.median)) | \(day.count) |")
        }
        
        md.append("")
        md.append("---")
        md.append("*Generated by Response Time*")
